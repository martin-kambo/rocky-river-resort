...
let ResendEmailProvider = ResendEmailProvider_1 = class ResendEmailProvider {
    constructor(config) {
        this.config = config;
        this.logger = new common_1.Logger(ResendEmailProvider_1.name);
        this.client = new resend_1.Resend(config.getOrThrow('RESEND_API_KEY'));
        this.from = config.get('EMAIL_FROM', 'Rocky River Resort <hello@rockyriverresort.co.ke>');
    }
